...
public class HDFSReader extends AbstractFileReader {
    ...
    protected Readers openReaders() throws IOException {
        ...
        Readers readers = new Readers(this.source());
        if (this.hdfs.isFile(path)) {
            readers.addReadable(new ReadablePath(this.hdfs, path));
        } else {
            assert this.hdfs.isDirectory(path);
            FileStatus[] statuses = this.hdfs.listStatus(path);
            Path[] paths = FileUtil.stat2Paths(statuses);
            ReadablePath[] readablePaths = new ReadablePath[paths.length];
            for (int i = 0; i < paths.length; i++) {
                readablePaths[i] = new ReadablePath(this.hdfs, paths[i]);
            }
            readers.addReadable(readablePaths);
        }
        return readers;
    }
