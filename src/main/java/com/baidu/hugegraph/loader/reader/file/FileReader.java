...
public class FileReader extends AbstractFileReader {
    ...
    protected Readers openReaders() throws IOException {
        ...
        if (file.isFile()) {
            return new Readers(this.source(), new ReadableFile(file));
        } else {
            ...
            ReadableFile[] readableFiles = new ReadableFile[subFiles.length];
            for (int i = 0; i < subFiles.length; i++) {
                readableFiles[i] = new ReadableFile(subFiles[i]);
            }
            return new Readers(this.source(), readableFiles);
        }
    }
